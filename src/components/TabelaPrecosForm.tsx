...
export function TabelaPrecosForm({ onBack, onSuccess, editingTabelaPrecos }: TabelaPrecosFormProps) {
  ...
  const availableResiduos = residuos.filter((r) => !selectedResiduos.some((sr) => sr.id_residuo === r.id_residuo));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await supabase.from("tabela_precos_residuo").delete().eq("id_tabela_preco", tabelaId);
      } else {
        const { data, error } = await supabase
          .from("tabela_precos")
          .insert({
      ...
      const { error: residuosError } = await supabase.from("tabela_precos_residuo").insert(residuosInsert);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center space-y-0 pb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-semibold">{isEditing ? "Editar Tabela de Preços" : "Tabela de Preços"}</h2>
        </div>
      </CardHeader>
      <CardContent>
                  ...
                  >
                    <div>
                      ...
                      <Badge variant="secondary" className="ml-2">
                        R${" "}
                        {residuo.vlr_total.toLocaleString("pt-BR", {
