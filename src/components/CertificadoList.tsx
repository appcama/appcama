...
export function CertificadoList({ onAddNew, onEdit }: CertificadoListProps) {
  ...
  const filteredCertificados = certificados.filter(cert => {
    ...
    const matchesDataInicio = !dataInicio || 
      new Date(cert.dat_periodo_inicio) >= new Date(dataInicio.setHours(0, 0, 0, 0));

    // Filtro por data final
    const matchesDataFim = !dataFim || 
      new Date(cert.dat_periodo_fim) <= new Date(dataFim.setHours(23, 59, 59, 999));

    // Filtro por entidade
    const matchesEntidade = !entidadeId || 
      cert.id_entidade?.toString() === entidadeId;
