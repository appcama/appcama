...
const ALL_STEPS = ["passo-1","passo-2","passo-3","passo-4","passo-5","passo-6","passo-7","passo-8","passo-9"];

export function ManualColeta() {
  const [isGenerating, setIsGenerating] = useState(false);
  const [openSteps, setOpenSteps] = useState<string[]>(["passo-1"]);
  ...
  const handlePrint = async () => {
    ...
    const previousSteps = [...openSteps];
    setOpenSteps(ALL_STEPS);

    // Wait for DOM to update with all accordions open
    ...
    try {
      ...
      printWindow.document.write(`
        ...
        <html>
          <head>
            <title>Manual — Como Lançar uma Coleta | ReCiclaÊ</title>
            <meta charset="utf-8" />
            <style>
              ${cssText}
              ...
              /* Remove pointer-events-none for print readability */
              .pointer-events-none {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={handlePrint} disabled={isGenerating} className="gap-2">
