-- Reverter função authenticate_user para permitir login de usuários não validados
-- Eles serão redirecionados para tela de validação no frontend
...
BEGIN
  ...
  WHERE e.num_cpf_cnpj = cpf_cnpj
    ...
    AND u.des_status = 'A'
    AND u.des_locked = 'D';
    -- Removido: AND u.des_senha_validada = 'A'
    -- Permitimos login de usuários com senha não validada para redirecionamento
END;
$function$;